...
set ios_certificate=D:\Sepehr\MTeam Certifications\MTeam IOS Certificate.p12
...
set ios_mobprevision=ArousGramDist.mobileprovision
...
rem %adt% -package -target %android_target%  -storetype pkcs12 -storepass %android_pass% -keystore "%android_certificate%" "%name%.apk" "%name%-app.xml" "%name%.swf" %contents% %native_folder%


rem IOS export V
%adt% -package -target %ios_targ% %ios_useLegacy% -keystore "%ios_certificate%" -storetype pkcs12 -storepass %ios_pass%  -provisioning-profile  "%ios_mobprevision%"  "%name%-store.ipa" "%name%-app.xml"  "%name%.swf"  %contents% %ios_contents% %native_folder%